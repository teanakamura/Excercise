...
void simd_calc(int x, int y, int from, int to) {
  ...
  register __m256 reg1 = _mm256_loadu_ps(&data[from][y][x]);
  register __m256 reg2 = _mm256_loadu_ps(&data[from][y][x-1]);
  reg1 = _mm256_add_ps(reg1, reg2);
  reg2 = _mm256_loadu_ps(&data[from][y][x+1]);
  reg1 = _mm256_add_ps(reg1, reg2);
  reg2 = _mm256_loadu_ps(&data[from][y-1][x]);
  reg1 = _mm256_add_ps(reg1, reg2);
  reg2 = _mm256_loadu_ps(&data[from][y+1][x]);
  reg1 = _mm256_add_ps(reg1, reg2);
