...
void simd_calc(int x, int y, int from, int to) {
  int i;
  int copy_size = sizeof(float) * 8;
  float temp1[8], temp2[8], temp3[8], temp4[8], temp5[8];
  for (i = 0; i < 8; i++) {
    memcpy(temp1, &data[from][y][x+i], copy_size);
    memcpy(temp2, &data[from][y][x+i-1], copy_size);
    memcpy(temp3, &data[from][y][x+i+1], copy_size);
    memcpy(temp4, &data[from][y-1][x+i], copy_size);
    memcpy(temp5, &data[from][y+1][x+i], copy_size);
  }
